...
export class EnumLabelInterceptor implements NestInterceptor {
    ...
    private transformData(data: any): any {
        ...
        if (data && typeof data === 'object') {
            ...
            const result: any = {};
            const enumFields: { key: string; value: string; label: string }[] = [];
            const otherFields: { key: string; value: any }[] = [];
            ...
            for (const key in data) {
                ...
                if (typeof value === 'string' || typeof value === 'number') {
                    ...
                    if (label) {
                        enumFields.push({ key, value: value.toString(), label });
                    } else {
                        otherFields.push({ key, value });
                    }
                } else if (value && typeof value === 'object') {
                    otherFields.push({ key, value: this.transformData(value) });
                } else {
                    otherFields.push({ key, value });
                }
            }
            ...
            // First add non-enum fields
            for (const field of otherFields) {
                result[field.key] = field.value;
            }

            // Then add enum fields and their labels
            for (const enumField of enumFields) {
                result[enumField.key] = enumField.value;
                result[`${enumField.key}Label`] = enumField.label;
            }
