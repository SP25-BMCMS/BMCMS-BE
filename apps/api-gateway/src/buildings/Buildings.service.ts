...
export class BuildingsService {
  ...
  async getBuildingById(buildingId: string) {
    try {
      console.log("🚀 ~ Buildings2313212313Service ~ getBuildingById ~ buildingId:", buildingId);
      const buildingObservable = this.buildingsClient.send(BUILDINGS_PATTERN.GET_BY_ID, { buildingId });
      const building = await firstValueFrom(buildingObservable);
      console.log("🚀 ~ BuildingsService ~ getBuildingById ~ building:", building);
      return building;
    } catch (error) {
  ...
  async getApartmentWithBuilding(apartmentId: string) {
    try {
      console.log("🚀 ~ BuildingsService ~ getApartmentWithBuilding ~ apartmentId:", apartmentId);
      
      // First get the apartment to extract buildingId
      const apartmentResponse = await this.getApartmentByIdFromBuilding(apartmentId);
      console.log("🚀 ~ BuildingsService ~ getApartmentWithBuilding ~ apartmentResponse:", apartmentResponse);
      ...
      console.log("🚀 ~ BuildingsService ~ getApartmentWithBuilding ~ buildingId:", buildingId);
      
      // Get building details
      const buildingResponse = await this.getBuildingById(buildingId);
      console.log("🚀 ~ BuildingsService ~ getApartmentWithBuilding ~ buildingResponse:", buildingResponse);
      ...
      console.error("🚀 ~ BuildingsService ~ getApartmentWithBuilding ~ error:", error);
      throw new HttpException(
  ...
  async getApartmentByIdFromBuilding(apartmentId: string) {
    try {
      const apartmentObservable = this.buildingsClient.send('get_apartment_by_id', { apartmentId });
      const apartment = await firstValueFrom(apartmentObservable);
      return apartment;
    } catch (error) {
      console.error("🚀 ~ BuildingsService ~ getApartmentByIdFromBuilding ~ error:", error);
