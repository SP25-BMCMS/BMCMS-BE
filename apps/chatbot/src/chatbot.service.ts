...
export class ChatbotService {
  ...
  private async getCrackReportsInfo(userId: string): Promise<{ info: string, imageUrl: string }> {
    try {
      ...
      // Sort reports by created_at to find the most recent one
      const sortedReports = [...crackReports].sort((a, b) => {
        ...
        try {
          ...
          if (report.crackDetails && report.crackDetails.length > 0) {
            ...
            if (report.crackDetails[0].aiDetectionUrl) {
              ...
              // Lưu URL ảnh từ báo cáo gần nhất
              if (report === sortedReports[0] && report.crackDetails[0].aiDetectionUrl) {
                mostRecentImageUrl = report.crackDetails[0].aiDetectionUrl;
              }
            }
          }
          crackReportsInfo += `\n`;
      ...
      console.log("🚀 ~ ChatbotService ~ getCrackReportsInfo ~ Most recent image URL:", mostRecentImageUrl);
      return { info: crackReportsInfo, imageUrl: mostRecentImageUrl };
    } catch (error) {
  ...
  async testChat(message: string, userId: string): Promise<{ message: string, url_img: string }> {
    ...
    try {
      ...
      try {
        // Lấy thông tin crack reports
        const crackReportsResult = await this.getCrackReportsInfo(userId);
        const crackReportsInfo = crackReportsResult.info;
        imageUrl = crackReportsResult.imageUrl;
...
- Trả lời có cảm xúc với cư dân vinhomes, không cần súc tích , miễn là nói sao cho cư dân an tâm về vấn đề vết nứt 
...
- 
- Luôn luôn cho họ số điện thoại cần hỗ trợ "0939193974" nhớ ghi những câu như thế này không cần gập khuông "bạn có thắc mắc gì hay có gì khiếu nại hãy gọi tới ban quan lý "
- Nếu người dùng hỏi về tình trạng vết nứt, hãy tham khảo thông tin vết nứt của họ từ phần THÔNG TIN VẾT NỨT CỦA BẠN
- bởi vì 1 resident có rất nhiều bản report , nên dựa vào  ${crackReportsInfo} created_at mới nhất để trả về  , nhớ không cần trả ảnh AI detect, Trả về không được lấy từ hệ thống trả về mà phải trả sao cho nó tự nhiên nhất (ví dụ như Inprogress thì là đang trong quá trình), và hãy nói thật hay cho cư dân an tâm về vấn đề vết nứt
THÔNG TIN HỆ THỐNG:
...
Câu hỏi của người dùng: ${message}`;
      ...
      this.logger.log(`[testChat] Image URL: "${imageUrl}"`);
      return { message: response, url_img: imageUrl };
    } catch (error) {
      this.logger.error('[testChat] Error processing message:', error);
      return { message: 'Xin lỗi, có lỗi xảy ra khi xử lý tin nhắn của bạn.', url_img: "" };
    }
  }
...
//       try {
//         // Lấy thông tin crack reports
//         const crackReportsResult = await this.getCrackReportsInfo(request.userId);
//         const crackReportsInfo = crackReportsResult.info;
//         imageUrl = crackReportsResult.imageUrl;
